...
StadeWindow::StadeWindow(QWidget *parent)
    : QMainWindow(parent)
    , ui(new Ui::StadeWindow)
{
    ui->setupUi(this);

    // Set selection mode and behavior for tableViewStade
    ui->tableViewStade->setSelectionMode(QAbstractItemView::SingleSelection);
    ui->tableViewStade->setSelectionBehavior(QAbstractItemView::SelectRows);
    ...
    connect(ui->lineEdit_6, &QLineEdit::textChanged, this, &StadeWindow::on_lineEdit_6_textChanged);
...
void StadeWindow::on_add_button_clicked()
{
    // Get values from UI
    ...
    if (success) {
        ...
        QSqlQuery query;
        query.prepare("SELECT 1 FROM DUAL"); // Dummy query to access last error
        query.exec();
...
void StadeWindow::on_modifStadeButton_clicked()
{
    bool ok;
...
void StadeWindow::on_DeleteStadeButton_clicked()
{
...
void StadeWindow::on_tableViewStade_doubleClicked(const QModelIndex &index)
{
    int row = index.row();
...
void StadeWindow::on_lineEdit_6_textChanged(const QString &arg1)
{
...
void StadeWindow::on_trier_Button_clicked()
{
...
bool StadeWindow::sendStatusChangeSMS(const QString& stadeName, const QString& newStatus)
{
    // Twilio credentials
...
void StadeWindow::on_pushButton_clicked()
{
...
void StadeWindow::displayStadiumLocationStats()
{
...
void StadeWindow::on_tableViewStade_clicked(const QModelIndex &index)
{
    // Ensure the row is selected
    ui->tableViewStade->selectionModel()->select(index, QItemSelectionModel::Select | QItemSelectionModel::Rows);
