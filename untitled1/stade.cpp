...
bool Stade::ajouter() {
    ...
    // Get the next ID by finding the maximum ID_STADE and adding 1
    query.prepare("SELECT NVL(MAX(ID_STADE), 0) + 1 FROM stade");
    if (!query.exec() || !query.next()) {
        qDebug() << "Failed to get next ID:" << query.lastError().text();
        return false;
    }
    int newId = query.value(0).toInt();

    query.prepare("INSERT INTO stade (id_stade, nom_stade, capacite, localisation, statut) "
                  "VALUES (:id_stade, :nom_stade, :capacite, :localisation, :statut)");
    query.bindValue(":id_stade", newId);
    ...
    if (query.exec()) {
        this->id = newId; // Update the object's ID
        return true;
    } else {
        qDebug() << "Failed to insert stadium:" << query.lastError().text();
        return false;
    }
}
...
bool Stade::supprimer(int id) {
    ...
    // Delete the stadium from STADE (related MATCHE records will be deleted automatically due to ON DELETE CASCADE)
    query.prepare("DELETE FROM stade WHERE id_stade = :id");
    query.bindValue(":id", id);
    if (!query.exec()) {
        qDebug() << "Failed to delete stadium:" << query.lastError().text();
        return false;
    }
...
bool Stade::modifier(int id) {
    ...
    if (!query.exec()) {
        qDebug() << "Failed to update stadium:" << query.lastError().text();
        return false;
    }
    return true;
}
